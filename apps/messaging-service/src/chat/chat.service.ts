...
import { CreateChatSessionDto, SendChatMessageDto, ChatQueryDto, EndSessionDto, EscalateDto, ChatSender } from './dto';
...
export class ChatService {
  ...
  async addMessage(sessionId: string, userId: string, dto: SendChatMessageDto) {
    ...
    const [message] = await this.db.insert(chatMessages).values({
      sessionId,
      sender: 'user',
      message: dto.message,
    }).returning();
    ...
    await this.addBotMessage(sessionId, botResponse.message, botResponse.intent, botResponse.confidence);

    return message;
  }

  private async addBotMessage(sessionId: string, message: string, intent?: string, confidence?: number) {
    const [botMessage] = await this.db.insert(chatMessages).values({
      sessionId,
      sender: 'bot',
      message,
      intent,
  ...
  private async generateBotResponse(userMessage: string) {
    ...
    if (lowerMessage.includes('job') || lowerMessage.includes('career')) {
      return { message: 'I can help you find jobs! What type of role are you looking for?', intent: 'job_search', confidence: 0.85 };
    }
    if (lowerMessage.includes('resume') || lowerMessage.includes('cv')) {
      return { message: 'I can help with your resume. Would you like tips on improving it or help uploading?', intent: 'resume_help', confidence: 0.9 };
    }
    if (lowerMessage.includes('interview')) {
      return { message: 'Preparing for an interview? I can share tips or help you practice common questions.', intent: 'interview_prep', confidence: 0.88 };
    }
    if (lowerMessage.includes('apply') || lowerMessage.includes('application')) {
      return { message: 'I can guide you through the application process. Which job are you interested in?', intent: 'application_help', confidence: 0.82 };
    }

    return { message: 'I\'m here to help with your job search. You can ask about jobs, resumes, interviews, or applications.', intent: 'general', confidence: 0.5 };
  }

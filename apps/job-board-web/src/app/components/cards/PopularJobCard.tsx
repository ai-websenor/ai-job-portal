...
import { MdLocationOn, MdWork, MdOutlinePayments, MdOutlineWorkOutline } from 'react-icons/md';
    ...
    <div className="flex flex-col h-full max-w-[400px] border-1.5 border-gray-100 rounded-[32px] p-8 bg-white shadow-sm font-sans hover:shadow-md transition-shadow">
      <div className="flex-grow">
        <div className="flex items-center gap-4 mb-6">
          <div className="relative w-14 h-14 rounded-full overflow-hidden bg-black shrink-0 flex items-center justify-center">
            {companyLogo ? (
              <Image src={companyLogo} alt={companyName} fill className="object-cover" />
            ) : (
              <MdOutlineWorkOutline className="text-3xl text-gray-400" />
            )}
          </div>
          <div className="flex flex-col">
            <h3 className="text-lg font-bold text-gray-800 leading-tight">{companyName}</h3>
        ...
        <div className="space-y-3 mb-6">
          <h2 className="text-2xl font-bold text-gray-900 leading-[1.2] tracking-tight">{title}</h2>
        ...
        <div className="space-y-3 mb-6">
          <div className="flex items-center gap-3 text-gray-600">
            ...
            <span className="text-[15px] font-medium">{CommonUtils.keyIntoTitle(role)}</span>
          </div>
          <div className="flex items-center gap-3 text-gray-600">
      ...
      <div className="flex items-center justify-between mt-auto">
        <Button color="primary" onPress={() => router.push(routePaths.jobs.apply(id))}>
          Apply Now
        </Button>
